...
import vu.vpn.app.util.FileLogWriter
import vu.vpn.di.ApplicationScope
import vu.vpn.di.KERMIT_FILE_LOG_DIR_NAME
import vu.vpn.di.appModule
...
import vu.vpn.lib.usecase.NotificationAction
...
class MullvadApplication : Application() {
    override fun onCreate() {
        ...
        // PSYCO · pre-warm do WebView (Chromium service) · força inicialização
        // do engine antes do Splash precisar dele · reduz a "tela preta" entre
        // o cold start do app e o globo R3F aparecer no SplashScreen.
        try {
            val warmup = android.webkit.WebView(this)
            warmup.destroy()
        } catch (e: Exception) {
            Logger.w("WebView pre-warm falhou: ${e.message}")
        }
        with(getKoin()) {
                ...
                .penaltyListener(executor) { violation ->
                    ...
                    val ignore =
                        violation.stackTrace.any {
                            it.className == "vu.vpn.app.service.MullvadVpnService"
                        }
                    if (ignore) {
